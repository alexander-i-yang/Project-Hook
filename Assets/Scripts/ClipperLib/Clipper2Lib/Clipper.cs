...
namespace Clipper2Lib
{
  ...
  public static class Clipper
  {
    ...
    //Returns true when p0 contains p1
    public static bool ContainsOneWay(Path64 p0, Path64 p1, double areaTolerance = 0.01f)
    {
      foreach (var p in p1)
      {
        if (PointInPolygon(p, p0) != PointInPolygonResult.IsInside) return false;
      }
      return true;
    }

    //Returns true when p0 contains p1 or vice versa.
    public static bool Contains(Path64 p0, Path64 p1, double areaTolerance = 0.01f)
    {
      return ContainsOneWay(p0, p1, areaTolerance) || ContainsOneWay(p1, p0, areaTolerance);
    }
    ...
    public static Path64 PointsToPath(Vector2[] p1, Vector2 offset) {
      List<int> path1 = new();
      foreach (var v in p1)
      {
        var tempV = v;
